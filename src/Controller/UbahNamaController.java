...
public class UbahNamaController implements Initializable {
 ArrayList<Pengguna> pengguna = new ArrayList<>();
    ...
    @FXML
    private TextField namaLama;
    
    @FXML
    private TextField namaBaru;
    ...
    private void bthChange(ActionEvent event){
            String newName = namaBaru.getText();
            String oldName = namaLama.getText();
            
            openTabel();
                if(pengguna.get(dp.getIndex()).getNama().equals(oldName)){
                    pengguna.get(dp.getIndex()).setNama(newName);
                    simpanTable();
